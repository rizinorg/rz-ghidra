...
Datatype *RizinTypeFactory::addRizinStruct(RzBaseType *type,
					   ...
					   bool prototype)
{
	...
	try {
		...
		void *it;
		rz_vector_foreach_cpp<RzTypeStructMember>(
			&type->struct_data.members,
			[&](RzTypeStructMember *member) {
				if (!member->type || !member->name)
					return;
				Datatype *member_type = fromRzType(
					member->type, nullptr, &stack_types);
				if (!member_type) {
					char *tstr = rz_type_as_string(
						core->analysis->typedb,
						member->type);
					arch->addWarning(
						std::string(
							"Failed to match type ") +
						(tstr ? tstr : "?") +
						" of member " + member->name +
						" in struct " + type->name);
					rz_mem_free(tstr);
					return;
				}
				...
				TypeField tf = {
					...
					std::string(member->name), member_type
				};
				fields.push_back(tf);
		...
		arch->addWarning(std::string("Failed to load struct ") +
				 type->name);
		return nullptr;
	}
}
...
Datatype *RizinTypeFactory::addRizinEnum(RzBaseType *type)
{
	...
	rz_vector_foreach_cpp<RzTypeEnumCase>(
		&type->enum_data.cases, [&](RzTypeEnumCase *ceys) {
			if (!ceys->name)
				return;
			nmap.emplace(ceys->val, ceys->name);
		});
	if (nmap.empty())
		return nullptr;
	try {
		...
		arch->addWarning(std::string("Failed to load enum ") +
				 type->name + ", " + e.explain);
		return nullptr;
	}
}

Datatype *RizinTypeFactory::addRizinTypedef(RzBaseType *type,
					    StackTypes &stack_types)
{
	assert(type->kind == RZ_BASE_TYPE_KIND_TYPEDEF);
	if (!type->type)
		return nullptr;
	Datatype *resolved = fromRzTypeInternal(
		type->type, nullptr, &stack_types, true,
		false); // use prototype=true to avoid recursion
	if (!resolved)
		return nullptr;
	Datatype *typedefd = getTypedef(resolved, type->name, 0, 0);
	fromRzTypeInternal(
		type->type, nullptr, &stack_types, false,
		false); // fully create the type after querying with prototype=true before
	return typedefd;
}
...
Datatype *RizinTypeFactory::addRizinAtomicType(RzBaseType *type,
					       StackTypes &stack_types)
{
	assert(type->kind == RZ_BASE_TYPE_KIND_ATOMIC);
	if (!type->name || type->size < 8) {
		arch->addWarning(std::string("Invalid atomic type ") +
				 (type->name ? type->name : "(null)"));
		return nullptr;
	}
	RzCoreLock core(arch->getCore());
	type_metatype mt = metatypeOfTypeclass(
		rz_base_type_typeclass(core->analysis->typedb, type));
...
Datatype *RizinTypeFactory::queryRizin(const string &n, StackTypes &stack_types,
				       bool prototype)
{
	...
	RzCoreLock core(arch->getCore());
	RzBaseType *type =
		rz_type_db_get_base_type(core->analysis->typedb, n.c_str());
	if (!type || !type->name)
		goto beach;
	switch (type->kind) {
...
Datatype *RizinTypeFactory::findById(const string &n, uint8 id, int4 sz,
				     StackTypes &stack_types, bool prototype)
{
...
Datatype *RizinTypeFactory::fromRzTypeInternal(const RzType *ctype,
					       string *error,
					       StackTypes *stack_types,
					       bool prototype, bool refd)
{
	switch (ctype->kind) {
	case RZ_TYPE_KIND_IDENTIFIER: {
		if (ctype->identifier.kind == RZ_TYPE_IDENTIFIER_KIND_UNION) {
			if (error)
				*error =
					"Union types not supported in Decompiler";
			return nullptr;
		}

		Datatype *r = stack_types ? findByName(ctype->identifier.name,
						       *stack_types,
						       prototype && refd) :
					    findByName(ctype->identifier.name);
		if (!r) {
			if (error)
				*error = "Unknown type identifier " +
					 std::string(ctype->identifier.name);
			return nullptr;
		}
		if (ctype->identifier.kind == RZ_TYPE_IDENTIFIER_KIND_STRUCT &&
		    r->getMetatype() != TYPE_STRUCT) {
			if (error)
				*error = "Type identifier " +
					 std::string(ctype->identifier.name) +
					 " is not the name of a struct";
			return nullptr;
		}
		if (ctype->identifier.kind == RZ_TYPE_IDENTIFIER_KIND_ENUM &&
		    !r->isEnumType()) {
			if (error)
				*error = "Type identifier " +
					 std::string(ctype->identifier.name) +
					 " is not the name of an enum";
			return nullptr;
		}
		return r;
	}
	case RZ_TYPE_KIND_POINTER: {
		Datatype *sub = fromRzTypeInternal(ctype->pointer.type, error,
						   stack_types, prototype,
						   true);
		if (!sub)
			return nullptr;
		auto space = arch->getDefaultCodeSpace();
		return this->getTypePointer(space->getAddrSize(), sub,
					    space->getWordSize());
	}
	case RZ_TYPE_KIND_ARRAY: {
		Datatype *sub = fromRzTypeInternal(
			ctype->array.type, error, stack_types, prototype, refd);
		if (!sub)
			return nullptr;
	...
	case RZ_TYPE_KIND_CALLABLE: {
		...
		ProtoModel *pm =
			callable->cc ?
				arch->protoModelFromRizinCC(callable->cc) :
				nullptr;
		if (!pm) {
			RzCoreLock core(arch->getCore());
		...
		if (!pm) {
			RzCoreLock core(arch->getCore());
			char *tstr = rz_type_as_string(core->analysis->typedb,
						       ctype);
			if (error) {
				*error =
					std::string(
						"Failed to get any calling convention for callable ") +
					tstr;
			}
			rz_mem_free(tstr);
			return nullptr;
		...
		if (callable->ret) {
			outtype = fromRzTypeInternal(callable->ret, error,
						     stack_types, prototype,
						     refd);
			if (!outtype)
				return nullptr;
		...
		if (!rz_pvector_foreach_cpp<RzCallableArg>(
			    callable->args, [&](RzCallableArg *arg) {
				    if (!arg->type)
					    return false;
				    Datatype *at = fromRzTypeInternal(
					    arg->type, error, stack_types,
					    prototype, refd);
				    if (!at)
					    return false;
		...
		PrototypePieces pieces{ .intypes = intypes,
					.model = pm,
					.outtype = outtype };
		return this->getTypeCode(
			pieces); // dotdotdot arg can be used when rizin supports vararg callables
	}
	}
	return nullptr;
}

Datatype *RizinTypeFactory::fromRzType(const RzType *ctype, string *error,
				       StackTypes *stack_types)
{
